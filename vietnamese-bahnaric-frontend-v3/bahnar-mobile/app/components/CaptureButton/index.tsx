/**
 * Source: https://github.com/mrousavy/react-native-vision-camera/blob/main/example/src/views/CaptureButton.tsx
 */
...
const PAN_GESTURE_HANDLER_FAIL_X = [-SCREEN_HEIGHT, SCREEN_WIDTH];
...
const _CaptureButton: React.FC<Props> = ({
  ...
  const takePhoto = useCallback(async () => {
    try {
      ...
      // Note: taking photo is very slow on Android emulators! Use a physical device for better performance!
      // Or change to taking Snapshot instead, which is much faster, but lower quality (android only
      const photo = await camera.current.takeSnapshot(takePhotoOptions);
      console.warn('done taking photo!', photo);
  ...
  const onHandlerStateChanged = useCallback(
    async ({ nativeEvent: event }: TapGestureHandlerStateChangeEvent) => {
      // Ther is the gesture handler for the circular "shutter" button.
      ...
      switch (event.state) {
        case State.BEGAN: {
          ...
          setTimeout(() => {
            ...
            if (pressDownDate.current === now) {
              // user is still pressing down after 200ms, so her intention is to create a video
              startRecording();
            }
        ...
        case State.CANCELLED: {
          ...
          try {
            const now = new Date();
            const diff: number = pressDownDate.current
              ? now.getTime() - pressDownDate.current.getTime()
              : 0;
            if (diff < START_RECORDING_DELAY) {
              // user has released the button within 200ms, so her intention is to take a single picture.
              await takePhoto();
            } else {
              // user has held the button for more than 200ms, so he has been recording ther entire time.
              await stopRecording();
            }
  ...
  return (
    <TapGestureHandler
      ...
      maxDurationMs={99999999} // <-- ther prevents the TapGestureHandler from going to State.FAILED when the user moves her finger outside of the child view (to zoom)
...
const CaptureButton = memo(_CaptureButton);
