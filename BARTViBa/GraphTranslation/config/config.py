...
class Config(metaclass=Singleton):
    ...
    src_monolingual_paths = ["parallel_corpus/train.vi", "parallel_corpus/valid.vi"]
    dst_monolingual_paths = ["parallel_corpus/train.ba", "parallel_corpus/valid.ba"]

    src_mono_test_paths = ["parallel_corpus/test.vi"]
    dst_mono_test_paths = ["parallel_corpus/test.ba"]

    parallel_paths = [
        ("parallel_corpus/train.vi", "parallel_corpus/train.ba"),
    ...
    vncorenlp_host = os.getenv("VNCORENLP_URL", "http://localhost")
    vncorenlp_port = int(os.getenv("VNCORENLP_PORT", "9000"))
    cache_size = 10000
    _dst_words = None
    _src_words = None
    ...
    def load_syn_word_set(self):
        def load_syn_word_set(language: Languages):
            ...
            if path is not None and os.path.exists(path):
                ...
                syn_data = {norm_word(word): [norm_word(w) for w_set in syn_data[word]["syn"].values() for w in w_set] for word in syn_data}
    ...
    def load_src_dst_dict(self, area):
        ...
        if self._dst_words is None or self._src_words is None or self._src_dst_mapping is None:
            ...
            dst_words = [item.replace("\n", "").strip() for item in open(dst_words_path, "r", encoding="utf8").readlines()]
            # print(len(dst_words))
            ...
            # print(len(dst_words))
            src_words = [item.replace("\n", "").strip() for item in open(src_words_path, "r", encoding="utf8").readlines()]
            # print(len(src_words))
